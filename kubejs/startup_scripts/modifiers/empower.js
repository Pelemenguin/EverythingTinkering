...
/**
 * - Checks if a item can be accepted by `empower`.
 * - 检查一个物品是否能被`充能`接受。
 * - - - - -
...
let EMPOWER = ModifierManager.registerCommonModifier("empower", "EmpowerModifier", {
    onProjectileLaunch: (tool, modifier, shooter, ammo, projectile /*, arrow, persistent, isPrimary */) => {
        ...
        shooter.inventory.getAllItems().forEach(stack => {
            ...
            projectile.mergeNbt({ForgeData: {IsLaunchedByEmpower: NBT.byteTag(1)}});
        });
    },
    __custom__: {
        onServerTick: (event) => {
            event.getServer().getEntities().forEach(entity => {
                ...
                /** @type {Internal.ByteTag} */ let shouldAddParticles = forgeData.get("IsLaunchedByEmpower");
