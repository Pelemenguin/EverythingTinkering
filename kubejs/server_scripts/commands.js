            ...
            .then(Commands.literal('parts')
                .then(Commands.argument('targets', Arguments.PLAYERS.create(event))
                    .then(Commands.argument('materialId', Arguments.STRING.create(event))
                        ...
                        .executes(command => {
                            const targets = Arguments.PLAYERS.getResult(command, 'targets')
                            const materialId = Arguments.STRING.getResult(command, 'materialId')
                            ...
                            targets.forEach(target => {
                                ...
                                success++
                                // console.info(`give @s tconstruct:part_chest${new_nbt}`)
                            })
                            return success
                    }))
                )
